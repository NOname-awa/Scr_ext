	...
	*/
	function getConfig(type) {
		const configs = {
			...
			SuzhouNumerals: {
				nums: ['〇', ['〡', '一'], ['〢', '二'], ['〣', '三'], '〤', '〥', '〦', '〧', '〨', '〩'],
				intRadice: '',
				intUnits: '',
				decUnits: '点',
				minus: '負',
	...
	*/
	function convertIntegerPart(strInt, config) {
		...
		for (let i = 0; i < len; i++) {
			...
			if (digit === '0') {
				zeroCount++;
				if (i === len - 1 && !config.units) {
					result += config.nums[0];
				}
			} else {
				if (zeroCount > 0) {
			...
			if (config.units && radicePosition === 0 && zeroCount < config.partition) {
				...
				menus: {
					type: {
						...
						items: [
							...
							{
								text: lang('SuzhouNumerals', 'Suzhou numerals'),
								value: 'SuzhouNumerals'
							}
						]
					}
				}
